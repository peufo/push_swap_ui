...
<fieldset class="border rounded p-4">
    ...
    <div class="flex gap-4 pb-4 items-center">
        <div class="flex gap-2">
            <button class="btn" class:outline={mode === 'h'} onclick={() => mode = 'h'}>
                Horizontal
            </button>
            <button class="btn" class:outline={mode === 'v'} onclick={() => mode = 'v'}>
                Vertical
            </button>
        </div>
        <div>
    ...
    {:else}
        ...
        >
            {#each stack.values.slice(stack.cursor) as value, index (value)}
                {@const w = Math.max((width / 2) * (value / max) - paddingH, 1)}
                <rect
                    width={w}
                    height={H - paddingW}
                    x={(width * 0.25) - (w * 0.5)}
